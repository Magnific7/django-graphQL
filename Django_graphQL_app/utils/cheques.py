...
def cheques_inwards(ripps_clean,ledger):
    ...
    bk_merge_cheque_0808 = pd.merge(cheque_ripps,ledger, how='right',left_on='Reference',right_on='instnID',)
    print('bk side',bk_merge_cheque_0808.shape)
    ...
    cheque_matching = bk_merge_cheque_0808[bk_merge_cheque_0808['Reference'] == bk_merge_cheque_0808['instnID']]
    print(cheque_matching.shape)
    legder_cheque_mismatch = bk_merge_cheque_0808[bk_merge_cheque_0808['Reference'] != bk_merge_cheque_0808['instnID']]
...
def outward(Stmnt,Ripps):
    ...
    BK_merge = pd.merge(Stmnt,ripps, how='left', left_on='NAreference', right_on='Breference')
    ...
    mismatch_bk = BK_merge[(BK_merge['NAreference']!=BK_merge['Breference'])  ]
    print('BNR merged:',BNR_merge.shape)
    print('BK merged:' ,BK_merge.shape)
    print('matched:' ,match_bnr.shape)
    print('BNR mismatch:' ,mismatch_bnr.shape)
    print('BK mismatch:' ,mismatch_bk.shape)
    return match_bnr,mismatch_bnr,mismatch_bk
